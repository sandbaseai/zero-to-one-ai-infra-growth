...
Related draft: Uneed waiting-line product draft, slug `sandbase`

## Goal

Prepare the launch assets before scheduling SandBase on Uneed.

The launch should not read like a generic AI tool listing. It should reinforce one idea:
...
Use 3-5 clean screenshots. Avoid dashboards with private data, API keys, emails, admin panels, or unfinished placeholder text.

Recommended set:

1. Homepage hero showing the core positioning.
2. Docs or quickstart showing developers how to start.
3. Models/tools page showing infrastructure breadth.
4. Agent Ecosystem / registry page if live.
5. Hackathon Support page if live and polished.

Screenshot rules:

- crop browser chrome unless the URL helps trust
- use light theme if the UI is easier to read
- keep text readable at small card size
...
### If someone asks what SandBase is
...
### If someone asks how it differs from an agent framework
...
### If someone asks why sandboxing matters
...
### If someone asks whether this is only for one model provider
...
- "secure by default" unless a specific security claim is documented
- "enterprise ready" without proof
- fake traction, customers, benchmarks, or integrations

## Schedule Checklist

Schedule Uneed only when:

- screenshots are ready
- first comment is ready
- X support post is ready
- LinkedIn support post is ready
- Discord update is ready
- one person is available to reply during launch day
- no paid upgrade is selected without explicit confirmation
- product positioning still matches the website, docs, GitHub, LinkedIn, Peerlist, and Uneed draft
